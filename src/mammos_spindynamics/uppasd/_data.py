...
class MammosUppasdData:
    ...
    def __init__(self, out: pathlib.Path):
        ...
        for dir_ in self.out.iterdir():
            if re.match(r"(run|temperature_sweep)-\d+", dir_.name):
                ...
                runs.append(
                    {"name": dir_.name, **info["metadata"], **info["parameters"]}
                )
        self._runs = runs
    ...
    def __getitem__(self, idx):
        ...
        if self._runs[idx]["mode"] == "run":
            return RunData(self.out / self._runs[idx]["name"])
        elif self._runs[idx]["mode"] == "temperature_sweep":
            return TemperatureSweepData(self.out / self._runs[idx]["name"])
        else:
...
class RunData:
    ...
    #     # TODO: Energy fails for the moment
...
class TemperatureSweepData:
    ...
    def save_output(self, out: pathlib.Path | str | None = None) -> None:
        ...
        # TODO: just stack last line of each cumulant plus Temperature column
        # np.savetxt(
        #     out / "M(T)",
